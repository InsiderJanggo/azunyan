...
module.exports.load = async(bot) => {
    ...
    .get('/', function(req, res){
        res.render(__dirname + "/views/index.ejs", {
            status: (req.isAuthenticated() ? `${req.user.username}#${req.user.discriminator}` : "Connected"),
            bot: bot.user,
            user: req.user,
    ...
    .get("/servers/:guildID", CheckAuth, function(req, res) {
        ...
        if (!serv) return res.redirect(`https://discordapp.com/oauth2/authorize?client_id=${process.env.CLIENTID}&scope=bot&permissions=0&guild_id${req.params.guildID}`);

        if(!bot.guilds.cache.get(req.params.guildID).members.get(req.user.id).hasPermission("MANAGE_GUILD")) return res.redirect("/dashboard");
    ...
    .post("/servers/:guildID", CheckAuth, function(req, res) {
        if(!req.body.send_CHANNELID) return res.send("Error, Please specify the channel")

        if(!req.body.send_MESSAGE) return res.send("Error, Message not specify");
        
        bot.guilds.cache.get(req.params.guildID).channels.get(req.body.send_CHANNELID).send(req.body.send_MESSAGE);
